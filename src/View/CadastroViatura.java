 ...
 * @author rjose
 */
public final class CadastroViatura extends javax.swing.JFrame {

    int t = 0;

    List<Entradaviatura> tipoProd = new ArrayList<>();
    EntityManagerFactory emf = new metodos.ControllerAcess().getEntityManagerFactory();
    DefaultTableModel dTableModel = new DefaultTableModel(null, new String[]{"id", "Matricula", "Modelo","Cor","Cliente", "Data. Entrou", "Data Saiu", "Manutenc.","Editar", "Remover"});
    private int utiliz=0;
    @Override
    public void list() {
        super.list();
    }

    public CadastroViatura(int c) {
        initComponents();
        this.setLocation(40, 225);
        // this.setResizable(false);
        this.setTitle("Cadastro de Fornecedores");
        loadListShow(new EntradaviaturaJpaController(emf).findEntradaviaturaEntities(), "");
        ...
        Icon im = new javax.swing.ImageIcon(getClass().getResource("/Imagens/aplicativos.png"));
         ButtonColumn bcm = new ButtonColumn(this.TableProduto, manute, 7, im);
    ...
    public void loadListShow(List<Entradaviatura> lista, String nome) {
        while (dTableModel.getRowCount() > 0) {
            dTableModel.removeRow(0);
        }
        if (!nome.isEmpty()) {
            lista = new EntradaviaturaJpaController(emf).getMatriculaLike(nome);//new Consultas().formarProduto("%" + nome + "%");
        }
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            dTableModel.setValueAt(lista.get(i).getMatricula(), i, 1);
            dTableModel.setValueAt(lista.get(i).getModelo(), i, 2);
            dTableModel.setValueAt(lista.get(i).getCorviatura(), i, 3);
            dTableModel.setValueAt(lista.get(i).getCliente(), i, 4);
            dTableModel.setValueAt(new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm").format(lista.get(i).getDataentrada()), i, 5);
           if(lista.get(i).getDatasaida()!=null) data = new java.text.SimpleDateFormat("dd/MM/yyyy HH:mm").format(lista.get(i).getDatasaida());
            dTableModel.setValueAt(data, i, 6);
        }
    ...
    Action delete = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            int respo = JOptionPane.showOptionDialog(null, "{Pretende remover o registo da viatura?","ATENÇÃO!",JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,   
                null,new String[]{"SIM", "NÃO"},null);
            if(respo==1||respo == -1)return;
            Entradaviatura p = (Entradaviatura) ((DefaultTableModel) table.getModel()).getValueAt(modelRow, 0);
            Boolean b = new EntradaviaturaJpaController(emf).getExistManutencao(p);
            if (b) {
                try {
                  //  System.out.println(p.getIdfornecedor()+"");
                    new EntradaviaturaJpaController(emf).destroy(p.getIdentrada());
                } catch (NonexistentEntityException ex) {
                    Logger.getLogger(CadastroViatura.class.getName()).log(Level.SEVERE, null, ex);
                ...
                JOptionPane.showMessageDialog(null, "Esta viatura nao pode ser removida!,\n "
                        + "Primeiro remover as pecas utilizadas na manutencao","Atencao",JOptionPane.WARNING_MESSAGE);
            }
        }
    };
    Action editar = new AbstractAction() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            FormViatura f = new FormViatura(new javax.swing.JFrame(), p, "Editar os dados", true);
            f.setVisible(true);
            loadListShow(new EntradaviaturaJpaController(emf).findEntradaviaturaEntities(), "");
        }
    };
    
    Action manute = new AbstractAction() {
        @Override
        public void actionPerformed(ActionEvent e) {
            JTable table = (JTable) e.getSource();
            int modelRow = Integer.valueOf(e.getActionCommand());
            Entradaviatura p = (Entradaviatura) ((DefaultTableModel) table.getModel()).getValueAt(modelRow, 0);
            p = new EntradaviaturaJpaController(emf).findEntradaviatura(p.getIdentrada());
            CadastroManute v = null;
            try {
                v = new CadastroManute(p,utiliz);
            } catch (Exception ex) {
                Logger.getLogger(CadastroViatura.class.getName()).log(Level.SEVERE, null, ex);
            }
...
//            loadListShow(new ProdutoJpaController(emf).getProdAsc(), "");
    ...
    private void initComponents() {
        ...
        jLabel9.setText("Pesquise pela matricula ou nome do cliente");
        ...
        labelHistory.setText("Gestao de Viaturas");
    ...
    private void btSalvarProdActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSalvarProdActionPerformed
        FormViatura f = new FormViatura(new javax.swing.JFrame(), null, "Adicionar Viatura", true);
        f.setVisible(true);
        loadListShow(new EntradaviaturaJpaController(emf).findEntradaviaturaEntities(), "");
